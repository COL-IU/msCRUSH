...
int main (int argc, char *argv[]) {
  ...
  print(unknown_spectra_all[ii]);
  print(unknown_spectra_all[jj]);
  return 0;
}

void print(Spectrum s) {
  cout << "BEGIN IONS" << endl;
  cout << "PEPMASS=" << s._precursor_mz << endl;
  cout << "CHARGE=" << s._charge << "+" << endl;
  cout << "TITLE=" << s._title << endl;
  for (const auto& peak : s._filtered_peaks) {
